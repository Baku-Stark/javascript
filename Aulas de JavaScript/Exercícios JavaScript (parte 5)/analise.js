...
function analise() {
    if(isNumero(num.value) && ! inLista(num.value, valores)) {
        ...
        window.alert('Valor inválido ou ja foi inserido na lista!')
    }
    num.value = ''
    num.focus()
}
...
function final() {
    if(valores.length == 0) {
        ...
        media = soma / tot
        res.innerHTML = ''
        res.innerHTML += `<p>Ao todo, temos ${tot} número(s) no cadastro.</p>`
        ...
        res.innerHTML += `<p>A média dos valores digitados resultou em: ${media}</p>`
    }
}